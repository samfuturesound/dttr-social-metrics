-- mx_alert_payload stays read-only so it can be called any time to preview the
-- email without side effects. mx_alert_claim records the send, and FS-8 calls it
-- only after Postmark returns 200.
...
as $function$
  ...
  rows_html as (
    select coalesce(string_agg(
      ...
      ||   initcap(a.network) || ' ' || a.content_type
  ...
  recent_html as (
    select coalesce(
      ...
      || string_agg(
           ...
           || r.brand_name
           || '<span style="color:#8a8378;"> &middot; ' || initcap(r.network) || ' '
...
create or replace function public.mx_alert_claim()
returns jsonb
language plpgsql
security definer
set search_path to 'metrics', 'public'
...
begin
  insert into metrics.alert_sends
    (external_id, network, content_type, brand_id, views_at_send, multiple_at_send)
  select a.external_id, a.network, a.content_type, a.brand_id,
         a.views_now, a.views_multiple
  from metrics.alerts a
  on conflict (external_id, network, content_type, sent_on) do nothing;

  get diagnostics n = row_count;
  return jsonb_build_object('claimed', n, 'claimed_on', current_date);
end;
$function$;

comment on function public.mx_alert_claim() is
  'Records that the posts currently in metrics.alerts have been emailed. Call only after the send succeeds. Safe to call twice in a day - the unique index absorbs it.';
